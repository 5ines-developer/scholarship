...
class auth extends CI_Controller {
    ...
    $this->security->xss_clean($_POST);
        ...
        if($this->session->userdata('sfn_id') != ''){ redirect('dashboard','refresh'); }
        if($this->input->post()){
                ...
                if ($this->form_validation->run() == True){
                    ...
                    if($result = $this->m_auth->can_login($email, $password)) 
                    {
                        $session_data = array(
                            ...
                            'sfn_name'      => $result['name'],
                            'sfn_type'      => $result['type']
                        ); 
    ...
    {
        $session_data = array(
            'sfn_mail'      => $this->session->userdata('sfn_mail'),
            'sfn_id'        => $this->session->userdata('sfn_id'),
            'sfn_status'    => $this->session->userdata('sfn_status'),
            'sfn_name'      => $this->session->userdata('sfn_name'),
            'sfn_type'      => $this->session->userdata('sfn_type')
        ); 
