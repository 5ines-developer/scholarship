...
class Sc_check {
	...
	public function loginSuccess($value='')
	{
		...
		$msg = '---------------------Success -- Student Logged in with '.$sess['slmail'].' and IP aaddress '.$ip.' --------------';
	...
	{
		$ip = $this->ci->input->ip_address();
		$msg = '---------------------Failed -- Student Log in Failed and tried login with '.$mail.' password and IP aaddress '.$ip.' --------------';
	...
	public function sus_mail($mail='')
	{
        ...
        $msg = $this->ci->load->view('email/suspic', $data, true);
    ...
    function limitRequests($ip='', $max_requests = 2, $sec = 5)
    {
        ...
        $this->ci->load->model('m_student');
        $exist = $this->ci->m_student->throttle_get($ips,$current_time);
        if(!empty($exist)){
            if($exist > 10){
                ...
                $exist = (int)$exist;
                $up = $exist + 1;
                $this->ci->m_student->throttle_update($ips,$up);
            }
        }else{
            $insert = array('ip' => $ips,'type'=>1,'created_at'=>date('Y-m-d'));
            $this->ci->m_student->throttle_insert($insert);
        }
    }
