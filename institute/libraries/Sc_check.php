...
class Sc_check {
    ...
    function limitRequests($ip='', $max_requests = 2, $sec = 5)
    {
        ...
        $this->ci->load->model('m_staffs');
        $exist = $this->ci->m_staffs->throttle_get($ips,$current_time);
        if(!empty($exist)){
            if($exist > 10){
                ...
                $exist = (int)$exist;
                $up = $exist + 1;
                $this->ci->m_staffs->throttle_update($ips,$up);
            }
        }else{
            $insert = array('ip' => $ips,'type'=>1,'created_at'=>date('Y-m-d'));
            $this->ci->m_staffs->throttle_insert($insert);
        }
    }
